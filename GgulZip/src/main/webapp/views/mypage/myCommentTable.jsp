...
<body>
	...
	<script>
		...
		$("#myCommentTable>tbody").on("click","tr td.title",function(){
	        ...
	        console.log(parentTr.find('.myCommentPostNo').val());
	        location.href = "${contextPath}/detail.bo?postNo="+parentTr.find('.myCommentPostNo').val();
		});
		...
		function appendToCommentTable(result){
			for(var i=0; i<result.length;i++){
                ...
                var postNo = $("<input type='text' class='myCommentPostNo' hidden value='"+result[i].postNo+"'>")
                tr.append(commentNo,category,content,writer,date,postTitle,postNo);
